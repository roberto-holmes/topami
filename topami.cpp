...
static std::unique_ptr<ExprAST> ParseParenExpr() {
	...
	if (CurTok != ')')
		return LogError("expected ')', read %i", CurTok);
	getNextToken();	 // eat ).
	return V;
}
...
static std::unique_ptr<ExprAST> ParseForExpr() {
	...
	if (CurTok != tok_identifier)
		return LogError("expected identifier after for, got %i", CurTok);

	std::string IdName = IdentifierStr;
	getNextToken();	 // eat identifier.

	if (CurTok != '=')
		return LogError("expected '=' after for, got %i", CurTok);
	getNextToken();	 // eat '='.
	...
	if (!Start)
		return nullptr;
	if (CurTok != ',')
		return LogError("expected ',' after for start value, got %i", CurTok);
	getNextToken();
	...
	if (CurTok != tok_in)
		return LogError("expected 'in' after for, got %i", CurTok);
	getNextToken();	 // eat 'in'.
...
static std::unique_ptr<ExprAST> ParseVarExpr() {
	...
	if (CurTok != tok_identifier)
		return LogError("expected identifier after var, got %i", CurTok);
	...
	while (true) {
		...
		if (CurTok != tok_identifier)
			return LogError("expected identifier list after var, got %i", CurTok);
	}

	// At this point, we have to have 'in'.
	if (CurTok != tok_in)
		return LogError("expected 'in' keyword after 'var', got %i", CurTok);
	getNextToken();	 // eat 'in'.
...
static std::unique_ptr<ExprAST> ParsePrimary() {
	switch (CurTok) {
		default:
			return LogError("unknown token when expecting an expression, got %i", CurTok);
		case tok_identifier:
...
static std::unique_ptr<PrototypeAST> ParsePrototype() {
	...
	switch (CurTok) {
		...
		case tok_unary:
			getNextToken();
			if (!isascii(CurTok))
				return LogErrorP("Expected unary operator, got %i", CurTok);
			FnName = "unary";
			FnName += (char)CurTok;
			Kind = 1;
			getNextToken();
		...
		case tok_binary:
			getNextToken();
			if (!isascii(CurTok))
				return LogErrorP("Expected binary operator, got %i", CurTok);
			FnName = "binary";
			FnName += (char)CurTok;
			Kind = 2;
			getNextToken();
			...
			if (CurTok == tok_number) {
				if (NumVal < 1 || NumVal > 100)
					return LogErrorP("Invalid precedence: must be 1..100, got %f", NumVal);
				BinaryPrecedence = (unsigned)NumVal;
				getNextToken();
			}
			break;
		default:
			return LogErrorP("Expected function name in prototype, got %i", CurTok);
	}

	if (CurTok != '(')
		return LogErrorP("Expected '(' in prototype, got %i", CurTok);
	...
	if (Kind && ArgNames.size() != Kind)
		return LogErrorP("Invalid number of operands for operator, got %i", CurTok);
...
Value* CallExprAST::codegen() {
	...
	if (CalleeF->arg_size() != Args.size())
		return LogErrorV("Incorrect # arguments passed, expected %u, got %u", Args.size(), CalleeF->arg_size());
	...
	if (argc == 1) {
		...
		for (const auto& entry : std::filesystem::directory_iterator(std::filesystem::current_path())) {
			if (entry.path().extension().compare(extension) == 0) {
				...
				std::string file_name_object = entry.path().stem().c_str();
				file_name_object += ".o";
				std::error_code EC;
				raw_fd_ostream dest(file_name_object.c_str(), EC, sys::fs::OF_None);
				...
				if (TheTargetMachine->addPassesToEmitFile(pass, dest, nullptr, FileType)) {
					outs() << "TheTargetMachine can't emit a file of this type";
					return 1;
				}
				...
				// Print out all of the generated IR code.
				TheModule->print(errs(), nullptr);

				outs() << "Wrote " << file_name_object << "\n";
...
/*
# Compile
clang++ -g -O3 topami.cpp dreamberd.cpp -I. `llvm-config --cxxflags --ldflags --system-libs --libs all` -o topami
